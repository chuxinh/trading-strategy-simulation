...
class TradingSimulation:
    ...
    def compute_portfolio(self):
        prices_bought_at, date = self._random_stock_pick(self.df, self.interval, self.price, self.same_start_date)
        ...
        # to calculae the cumulative sum of units purchased at a point in time
        total_units_at_each_trade = np.cumsum(num_units_each_trade)
        # to calculae the cumulative sum of units invested at a point in time
        total_dollar_invested_to_date = np.cumsum(np.full_like(total_units_at_each_trade, amount_invested_each_trade))
        ...
        result = pd.concat([self.df[self.price],  trading_record[["number_of_units", "invested_to_date"]]], axis=1)
    ...
    def _generate_end_value(self, current_price: float):
        prices_bought_at = self._random_stock_pick(self.df, self.interval, self.price, self.same_start_date)[0]
        amount_invested_each_trade = self.amount / len(prices_bought_at)
        num_units_each_trade = amount_invested_each_trade / prices_bought_at
        
        # to calculae number of units to date
        total_units_at_each_trade = np.sum(num_units_each_trade)
        
        return total_units_at_each_trade * current_price
    
    
    def run_simulation(self, current_price: float, n: int = 10000):
        simulated_results = []
        simulated_returns = []
        ...
        return simulated_results, simulated_returns
